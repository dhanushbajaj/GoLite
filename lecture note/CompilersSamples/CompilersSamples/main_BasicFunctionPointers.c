...
// exotic declarations
// foo is a function returning a pointer to array of pointer to function taking char and returning int
int (*(*foo(void))[])(char);

// arr is an array of 3 pointer to function taking no parameters returning pointer to array [5] of pointer to int
int * (*(*arr[3])(void))[5];

int main_BasicFunctionPointers(void) {
	// pointer to function taking two ints and returning int
	int (*ptr_f)(int,int);
