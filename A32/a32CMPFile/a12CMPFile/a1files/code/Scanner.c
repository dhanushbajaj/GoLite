...
----------------------------------------------------------------
*/
BufferPointer stringLiteralTable = NULL; // Or an appropriate initial value

GoLite_intg errorNumber = 0;
...
Token tokenizer(GoLite_void) {
	...
	GoLite_intg lexStart, lexEnd; // start and end offsets of a lexeme
	GoLite_intg lexLength; // token length
	Token currentToken = { 0 }; // token to return after recognition

	while (1) {
		c = readerGetChar(sourceBuffer); // read a character from the buffer
		...
		if (c == CHARSEOF0 || c == CHARSEOF255) {
			currentToken.code = SEOF_T; // Set token code for end-of-file
			return currentToken;
		}

		// Reset and determine initial state based on the character
		lexStart = readerGetPosRead(sourceBuffer) - 1;
		state = nextState(state, c);

		// Read characters and determine state until an accepting state is reached
		while (stateType[state] == NOFS) {
			c = readerGetChar(sourceBuffer);
			state = nextState(state, c);
		}

		// Perform action based on the final state
		lexEnd = readerGetPosRead(sourceBuffer);
		if (stateType[state] == FSWR) {
			lexEnd--; // retract if necessary
		...
		if (!lexemeBuffer) {
			// Error handling for buffer creation failure
			currentToken.code = ERR_T;
			strcpy(currentToken.attribute.errLexeme, "Buffer creation failed");
			return currentToken;
		}

		// Extract the lexeme from the source buffer
		readerRestore(sourceBuffer);
		for (int i = 0; i < lexLength; i++) {
			readerAddChar(lexemeBuffer, readerGetChar(sourceBuffer));
		}
		readerAddChar(lexemeBuffer, '\0'); // Null-terminate the lexeme

		// Generate the token based on the final state
		currentToken = (*finalStateTable[state])(readerGetContent(lexemeBuffer, 0));
		readerRestore(lexemeBuffer); // Reset lexeme buffer for next use
		return currentToken;
	}
}
...
GoLite_intg nextClass(rune c) {
	if (isalpha(c)) return CHRCOL_L;  // L (Letters)
	if (isdigit(c)) return CHRCOL_D;  // D (Digits)
	if (c == CHRCOL_U) return CHRCOL_U;  // U (Underscore)
	if (c == CHRCOL_Q) return CHRCOL_Q;  // Q (Double Quotation Marks)
	if (c == CHRCOL_q) return CHRCOL_q;  // q (Single Quotation Marks)
	if (c == CHRCOL_E) return CHRCOL_E;  // E (Exclamation Mark)
	if (c == CHRCOL_B) return CHRCOL_B;  // B (Forward Slash)
	if (c == CHRCOL_S) return CHRCOL_S;  // S (Star)
	if (c == CHARSEOF0 || c == CHARSEOF255) return 'E';  // EOF Characters

	return 'O';  // Default case for other characters
}
 ...
 ***********************************************************
 */
const char* tokenStrTable[] = {
	"ERR_T", "SEOF_T", "INL_T", "FPL_T", "STR_T",
	"MNID_T", "VID_T", "KW_T", "LPR_T", "RPR_T",
	"LBR_T", "RBR_T", "EOS_T", "CMT_T", "RTE_T"
