...
GoLite_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern GoLite_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern GoLite_intg line; /* source code line number - defined in scanner.c */
...
extern GoLite_void startParser(GoLite_void);
extern GoLite_intg startScanner(BufferPointer sc_buf);

static GoLite_void printParserError(GoLite_string fmt, ...);
static GoLite_void displayParser(BufferPointer ptrBuffer);
static GoLite_long getParserFileSize(GoLite_string fname);
static GoLite_void callGarbageCollector(GoLite_void);
...
GoLite_intg mainParser(GoLite_intg argc, GoLite_string* argv) {

	numParserErrors = 0;			/* Initializes the errors */

	FILE* fi;       /* input file handle */
	GoLite_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
GoLite_void printParserError(GoLite_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(GoLite_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
GoLite_long getParserFileSize(GoLite_string fname) {
	FILE* input;
	GoLite_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
GoLite_void displayParser(BufferPointer ptrBuffer) {
...
GoLite_void callGarbageCollector(GoLite_void) {
